...
class VCCompanyAccepted: UIViewController, IndicatorInfoProvider {

    var ordercollectionArray = [OrderFiltersCollection]()
    @IBOutlet weak var companyAcceptedTableView: UITableView!
    ...
    override func viewDidLoad() {
       ...
       self.companyGetOrderDetail()
        ...
        companyAcceptedTableView.addSubview(refreshControl)
    }
    
    
    
    fileprivate func setupDelegates(){
        self.companyAcceptedTableView.emptyDataSetSource = self as DZNEmptyDataSetSource
        self.companyAcceptedTableView.emptyDataSetDelegate = self as DZNEmptyDataSetDelegate
        self.companyAcceptedTableView.tableFooterView = UIView()
        self.companyAcceptedTableView.reloadData()
    }
...
extension VCCompanyAccepted: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell =  tableView.dequeueReusableCell(withIdentifier: "CellAcceptedCompany") as? CellAcceptedCompany
        ...
        print(ordercollectionArray[indexPath.row].policyPath)
        if(ordercollectionArray[indexPath.row].policyPath == ""){
            cell?.lblfile.textColor = #colorLiteral(red: 0.8642396331, green: 0.2060347497, blue: 0.2716235518, alpha: 1)
            cell?.lblfile.text = "Add Policy File".localized
        }else{
            cell?.lblfile.textColor = #colorLiteral(red: 0.5818647146, green: 0.8263530135, blue: 0.2647219598, alpha: 1)
            cell?.lblfile.text = "View Policy File".localized
        }
...
extension VCCompanyAccepted: DZNEmptyDataSetSource,DZNEmptyDataSetDelegate {
...
extension VCCompanyAccepted:companyacceptedProtocool{
    func myaccepted(cell: CellAcceptedCompany) {
        let indx = companyAcceptedTableView.indexPath(for: cell)
        let storyboard = UIStoryboard.init(name: "Main", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "VCQuotationLinks") as? VCQuotationLinks
